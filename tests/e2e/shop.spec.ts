...
test('complete delivery and pickup flow, validation, variants, persistence and message', async ({ page }) => {
  ...
  await expect(page.locator('.summary-total')).toContainText('$56.000');
  await page.reload();
  await expect(page.locator('.summary-total')).toContainText('$56.000');
  ...
  await expect(page.getByRole('link', { name: 'Abrir WhatsApp', exact: true })).toHaveAttribute('href', /^https:\/\/wa\.me\/573173560428\?text=/);
  ...
  await expect(message).toContainText('3 días hábiles');
  await expect(message).toContainText('$56.000');
...
test('stale catalog and unavailable cart are safely handled', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Revisar mi pedido' }).click();
  await page.evaluate(catalog => { catalog.products.find(p => p.id === 'pulsera-luna')!.price = 25000; localStorage.setItem('nala.catalog.v1', JSON.stringify(catalog)); }, structuredClone(seed));
  await page.getByRole('button', { name: /Preparar WhatsApp|Volver a verificar pedido/ }).click();
